...
def generate_list_row(list_row_data: Tuple[int, List[str]]) -> html.Div:
    ...
    Args:
        list_row_data (Tuple[int, List[str]]): The row index and content of the row
...
def generate_list_titles(list_titles: List[str]) -> List[html.Div]:
...
def dashboards_list_component(
    ...
    Args:
        titles_names (List[str]): The titles to display at the top of the list.
        list_rows (List[List[str]]): The rows that make up the list contents.

    Raises:
        IndexError: If the amount of titles does not match the amount of list rows.
